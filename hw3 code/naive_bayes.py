...
def train_mle_estimator(train_images, train_labels):
    ...
    num_label = np.sum(train_labels, axis=0)
    pi_mle = num_label / len(train_labels)
    theta_mle = np.dot(train_images.T, train_labels) / num_label
...
def train_map_estimator(train_images, train_labels):
    ...
    # YOU NEED TO WRITE THIS PART
    num_label = np.sum(train_labels, axis=0)
    pi_map = num_label / len(train_labels)
    theta_map = (np.dot(train_images.T, train_labels)+2) / (num_label+4)
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    log_p = np.dot(images, np.log(theta))+np.dot(1-images, np.log(1-theta))
    predict(np.log(pi) + log_p - np.log(np.dot(np.exp(log_p),pi)).reshape(-1, 1))
    return np.log(pi) + log_p - np.log(np.dot(np.exp(log_p),pi)).reshape(-1, 1)
...
def accuracy(log_like, labels):
    ...
    # YOU NEED TO WRITE THIS PART
    return np.mean(predict(log_like) == predict(labels))
